...
ROLE_CDN = (select ROLE_CDN from T_ORION_ROLE r
	...
	where
		r.NAME = 'Administrateur Local ARQ (ALA)'
...
ROLE_CDN = (select ROLE_CDN from T_ORION_ROLE r
	join T_ORION_APPLICATION a on a.APPLICATION_CDN = r.APPLICATION_CDN
	where
		r.NAME = 'Administrateur National Métier ARQ (ANMA)'
	and	a.NAME='Analyse de Risque'
);
...
ROLE_CDN = (select ROLE_CDN from T_ORION_ROLE r
	join T_ORION_APPLICATION a on a.APPLICATION_CDN = r.APPLICATION_CDN
	where
		r.NAME = 'Administrateur SIAL ARQ (ASA)'
	and	a.NAME='Analyse de Risque'
);
...
(Structure*Domaine Technique)',
		...
		ROLE_INHERITED_CDN=(select ROLE_CDN from T_ORION_ROLE r
			...
			and	a.NAME = null)
	where
		NAME='Administrateur Local ARQ (ALA)'
...
then
	insert into T_ORION_ROLE(
		...
		'Administrateur National Métier ARQ (ANMA)',
	...
	update T_ORION_ROLE set
		...
		ROLE_INHERITED_CDN=(select ROLE_CDN from T_ORION_ROLE r
			...
			and	a.NAME = null)
	where
		NAME='Administrateur National Métier ARQ (ANMA)'
	and	APPLICATION_CDN = (select APPLICATION_CDN from T_ORION_APPLICATION where NAME = 'Analyse de Risque');
end if;
...
technique.',
		...
		ROLE_INHERITED_CDN=(select ROLE_CDN from T_ORION_ROLE r
			...
			and	a.NAME = null)
	where
		NAME='Administrateur SIAL ARQ (ASA)'
	and	APPLICATION_CDN = (select APPLICATION_CDN from T_ORION_APPLICATION where NAME = 'Analyse de Risque');
end if;
...
and	g.NAME='Groupe de droit changement role'
and	r.NAME='Administrateur Local ARQ (ALA)';
...
where
	a.NAME='Analyse de Risque'
and	g.NAME='Groupe de droit changement role'
and	r.NAME='Administrateur National Métier ARQ (ANMA)';
...
where
	a.NAME='Analyse de Risque'
and	g.NAME='Groupe de droit changement role'
and	r.NAME='Administrateur SIAL ARQ (ASA)';
...
(
   (select role_cdn
	...
	where app.name='Analyse de Risque' and role.name='Consultant ARQ (CA)'
	),
...
(
   (select role_cdn
    from t_orion_role role
	join t_orion_application app on app.application_cdn = role.application_cdn
	where app.name='Analyse de Risque' and role.name='Administrateur Local ARQ (ALA)'
	),
   (select ac_id from attribut_complementaire where ac_code_rfa='STRUCT_DOMTECH!'),
   false
);
...
(
   (select role_cdn
    from t_orion_role role
	join t_orion_application app on app.application_cdn = role.application_cdn
	where app.name='Analyse de Risque' and role.name='Administrateur National Métier ARQ (ANMA)'
	),
...
(
   (select role_cdn
    from t_orion_role role
	join t_orion_application app on app.application_cdn = role.application_cdn
	where app.name='Analyse de Risque' and role.name='Administrateur SIAL ARQ (ASA)'
	),
   null,
   false
);
