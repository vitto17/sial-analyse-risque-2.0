 ...
 * @author pegaltier
 */
public class VEspeceDgalDAOImpl extends DefaultBusinessDAO implements VEspeceDgalDAO {
    
    private final static int MAX_ITEM_IN = 100;
    
    @Override
    public Map<String,VEspeceDgal> findWithListRfa(List<String> listRfaAll) throws Exception {      
        
        // Init
        QueryBuilder queryBuilder = new QueryBuilder();
        queryBuilder.from(VEspeceDgal.class);
        
        double nbIteration1= (double) ((double) listRfaAll.size())/ ((double) MAX_ITEM_IN);
        int nbIteration = (int) Math.ceil(nbIteration1);
        
        Collection<VEspeceDgal> collectionAll = new ArrayList<VEspeceDgal>();
        Map<String,VEspeceDgal> retour = new HashMap<String,VEspeceDgal>();
        ...
        for (int i = 0;i<=nbIteration-1;i++){
            ...
            Expression expression = new SimpleExpression(new Criterion("espdRfa"),
            ...
            Collection<VEspeceDgal> collection = super.findBy(businessSearchContext);

            collectionAll.addAll(collection);
        }

        for (VEspeceDgal especeDgal : collectionAll) {
            retour.put(especeDgal.getEspdRfa(), especeDgal);
        }
